...
const useSearchForm = () => {
  ...
  useEffect(() => {
    let nominationslist = JSON.parse(
  ...
  const handleAddNominate = (event, nominatedMovie) => {
    ...
    let movieListUpdated = movieList.map((movie) =>
      movie.imdbID === nominatedMovie.imdbID
  ...
  const handleRemoveNominate = (event, nominatedMovie) => {
    let newNominateList = nominations.filter(
      (nomination) => nomination.imdbID !== nominatedMovie.imdbID
    );
